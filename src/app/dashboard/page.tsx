      ...
      <main className="flex-1">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <Card className="relative mb-8 overflow-hidden border-none shadow-2xl">
            ...
            <div className="relative h-56 bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600">
              {/* Pattern overlay */}
              ...
              {/* Orbs décoratifs */}
              <div className="absolute top-0 right-0 w-64 h-64 bg-pink-400 rounded-full opacity-20 blur-3xl" />
              <div className="absolute bottom-0 left-0 w-64 h-64 bg-violet-400 rounded-full opacity-20 blur-3xl" />
                ...
                <div className="flex items-center gap-6 w-full">
                  ...
                  <div className="relative group">
                    ...
                    <Avatar className="relative h-28 w-28 border-4 border-white/30 shadow-2xl ring-4 ring-white/10 transition-transform group-hover:scale-105">
                      ...
                      <AvatarFallback className="text-4xl bg-gradient-to-br from-violet-500 to-fuchsia-500 text-white font-bold">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <Card className="border-0 shadow-lg bg-gradient-to-br from-purple-50 to-purple-100/50 hover:shadow-xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-purple-900/70">Votes reçus</p>
                    <p className="text-3xl font-bold text-purple-900 mt-1">{stats.votes}</p>
                    <p className="text-xs text-purple-700 mt-1">Continuez à partager !</p>
                  </div>
            ...
            <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-blue-100/50 hover:shadow-xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-blue-900/70">Mes photos</p>
                    <p className="text-3xl font-bold text-blue-900 mt-1">{stats.photos}</p>
                    <p className="text-xs text-blue-700 mt-1">Max 5 autorisé</p>
                  </div>
            ...
            <Card className="border-0 shadow-lg bg-gradient-to-br from-green-50 to-green-100/50 hover:shadow-xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-green-900/70">Participants</p>
                    <p className="text-3xl font-bold text-green-900 mt-1">{topParticipants.length}</p>
                    <p className="text-xs text-green-700 mt-1">Dans le concours</p>
                  </div>
            ...
            <Card className="border-0 shadow-lg bg-gradient-to-br from-amber-50 to-amber-100/50 hover:shadow-xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-amber-900/70">Classement</p>
                    <p className="text-3xl font-bold text-amber-900 mt-1">{stats.position > 0 ? `#${stats.position}` : "-"}</p>
                    <p className="text-xs text-amber-700 mt-1">{stats.position > 0 && stats.position <= 10 ? "Top 10 🔥" : "Gagnez des places !"}</p>
                  </div>
              ...
              <Card className="border-0 shadow-lg bg-gradient-to-br from-primary/5 to-purple-500/5">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Button asChild className="h-auto py-4 flex-col gap-2 btn-glow">
                      <Link href="/profile/edit">
                        <User className="h-6 w-6" />
                        <span className="font-medium">Modifier mon profil</span>
                      </Link>
                    </Button>
                    <Button asChild variant="outline" className="h-auto py-4 flex-col gap-2 hover:bg-purple-50">
                      <Link href="/vote">
                        <Vote className="h-6 w-6" />
                        <span className="font-medium">Voter maintenant</span>
                      </Link>
                    </Button>
                    <Button asChild variant="outline" className="h-auto py-4 flex-col gap-2 hover:bg-pink-50">
                      <Link href="/rules">
            ...
            <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-3">
                    <Avatar className="h-16 w-16 border-2 border-primary/20">
                      <AvatarImage src={getUserAvatar(currentUser.avatar)} alt={currentUser.name} />
              ...
              <Card className="border-0 shadow-lg">
                <CardHeader>
                  ...
                  <CardDescription>Votez pour vos participants favoris</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {suggested.length > 0 ? (
                      suggested.map((participant) => (
                        <div key={participant.id} className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-br from-muted/30 to-muted/10 hover:from-primary/5 hover:to-purple-500/5 transition-all group">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground truncate">{participant.style}</p>
                          </div>
                          <Button 
                            size="sm" 
                            variant="ghost" 
                            className="opacity-70 group-hover:opacity-100 hover:bg-primary/10 hover:text-primary transition-all"
                            asChild
                          >
                      ...
                      <p className="text-sm text-muted-foreground text-center py-6">Aucune suggestion</p>
                    )}
                  </div>
